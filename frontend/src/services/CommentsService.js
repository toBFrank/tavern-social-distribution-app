...
export const createComment = async (authorSerial, postSerial, commentData) => {
  try {
      const response = await api.post(`/authors/${authorSerial}/inbox`, commentData);
...
export const getComments = async (authorSerial, postSerial) => {
  try {
      const response = await api.get(`/authors/${authorSerial}/posts/${postSerial}/comments`);
      return response.data;
  } catch (error) {
      console.error(error);
  }
...
export const getCommentRemote = async (authorSerial, postSerial, remoteCommentFqid) => {
  try {
      const response = await api.get(`/authors/${authorSerial}/post/${postSerial}/comment/${remoteCommentFqid}`);
      return response.data;
  } catch (error) {
      console.error(error);
  }
...
export const getAuthorComment = async (authorSerial, commentSerial) => {
  try {
      const response = await api.get(`/authors/${authorSerial}/commented/${commentSerial}`);
      return response.data;
  } catch (error) {
      console.error(error);
  }
